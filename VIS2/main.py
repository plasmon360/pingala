...
word_length_div = Div(text="""<span class="display-2">""" +
                      str(slider_initial_val) + """</span>""",
                      name='word_length')
...
js_filter_n = CustomJSFilter(args=dict(
    slider=slider,
    source=source,
),
                             code='''
            var indices = [];
            console.log(source.data['word_length'])
...
js_filter_n_1 = CustomJSFilter(args=dict(
    slider=slider,
    source=source,
),
                               code='''
            ...
            return indices
''')
js_filter_n_2 = CustomJSFilter(args=dict(
    slider=slider,
    source=source,
),
                               code='''
...
not_ends_with_long_view = CDSView(
    source=source,
    filters=[
        BooleanFilter([not item for item in source.data['ends_with_long']]),
        js_filter_n
    ])

ends_with_long_view = CDSView(
    source=source,
    filters=[BooleanFilter(source.data['ends_with_long']), js_filter_n])


def myfig(name, view, height, width=400):
...
fig_n_1 = myfig(name="fig_n_1",
                view=CDSView(source=source, filters=[js_filter_n_1]),
                height=HEIGHT)

fig_n_2 = myfig(name="fig_n_2",
                view=CDSView(source=source, filters=[js_filter_n_2]),
                height=int(HEIGHT / 1.618))
fig_n_1.x_range = fig_n_not_long_ending.x_range
fig_n_2.x_range = fig_n_not_long_ending.x_range

curdoc().add_root(fig_n)
curdoc().add_root(fig_n_1)
curdoc().add_root(fig_n_2)
curdoc().add_root(slider)
curdoc().add_root(word_length_div)
curdoc().add_root(all_combinations)
